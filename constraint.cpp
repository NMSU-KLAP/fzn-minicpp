...
int* AllDifferentAC::getMatch()
{
    return  _match;
}

int AllDifferentAC::calcMatch()
{
    return _mm.compute(_match);
}

void AllDifferentAC::calcSCC()
{
    updateRange();
    updateGraph();
    _nc = 0;
    auto fun = [&](int n,int nd[])
        {
            for(int i=0;i < n;i++)
                _scc[nd[i]] = _nc;
            ++_nc;
...
void AllDifferentAC::filterDomains()
{
    for(int i=0;i < _nVar;i++)
        for(int v = _minVal; v <= _maxVal;v++)
            if (_match[i] != v && _scc[i] != _scc[valNode(v)])
            {
                _x[i]->remove(v);
            }
}
